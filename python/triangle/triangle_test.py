...
class DegenerateTriangleTest(unittest.TestCase):
    def test_no_sides_are_equal(self):
        self.assertIs(degenerate([1, 2, 3]), True)

    def test_all_sides_are_equal(self):
        self.assertIs(degenerate([4, 4, 4]), False)

    def test_may_not_violate_triangle_inequality(self):
        self.assertIs(degenerate([0, 0, 0]), False)

    def test_may_not_violate_triangle_inequality(self):
        self.assertIs(degenerate([4, 2, 1]), False)

    def test_may_not_violate_triangle_inequality(self):
        self.assertIs(degenerate([7, 7, 8]), False)

    def test_sides_may_be_floats(self):
        self.assertIs(degenerate([0.5, 0.4, 0.9]), True)
